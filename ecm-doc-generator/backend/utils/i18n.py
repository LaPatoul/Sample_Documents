"""
Internationalization utilities for document generation
"""

TRANSLATIONS = {
    'en-GB': {
        'invoice': 'Invoice',
        'purchase_order': 'Purchase Order',
        'receipt': 'Receipt',
        'order': 'Order',
        'delivery_note': 'Delivery Note',
        'payslip': 'Payslip',
        'employment_contract': 'Employment Contract',
        'expense_report': 'Expense Report',
        'invoice_number': 'Invoice No.',
        'po_number': 'PO No.',
        'receipt_number': 'Receipt No.',
        'order_number': 'Order No.',
        'delivery_note_number': 'Delivery Note No.',
        'payslip_number': 'Payslip No.',
        'contract_number': 'Contract No.',
        'report_number': 'Report No.',
        'date': 'Date',
        'due_date': 'Due Date',
        'order_date': 'Order Date',
        'delivery_date': 'Delivery Date',
        'bill_to': 'Bill To',
        'ship_to': 'Ship To',
        'vendor': 'Vendor',
        'customer': 'Customer',
        'receiver': 'Receiver',
        'description': 'Description',
        'quantity': 'Quantity',
        'unit': 'Unit',
        'unit_price': 'Unit Price',
        'amount': 'Amount',
        'total': 'Total',
        'subtotal': 'Subtotal',
        'tax': 'VAT',
        'tax_rate': 0.20,
        'grand_total': 'Grand Total',
        'status': 'Status',
        'payment_method': 'Payment Method',
        'items': 'Items',
        'order_items': 'Order Items',
        'items_shipped': 'Items Shipped',
        'weight': 'Weight',
        'package_number': 'Package No.',
        'phone': 'Phone',
        'email': 'Email',
        'receipt': 'Receipt',
        'category': 'Category',
        'sample_watermark': 'SAMPLE - FOR DEMONSTRATION PURPOSES ONLY',
        'thank_you': 'Thank you for your business',
        'company_info': 'Company Information',
        'customer_info': 'Customer Information',
        'pending': 'Pending',
        'approved': 'Approved',
        'received': 'Received',
        'confirmed': 'Confirmed',
        'processing': 'Processing',
        'shipped': 'Shipped',
        'submitted': 'Submitted',
        'rejected': 'Rejected',
        'reimbursed': 'Reimbursed',
        'permanent': 'Permanent',
        'fixed_term': 'Fixed Term',
        'internship': 'Internship',
        'cash': 'Cash',
        'credit_card': 'Credit Card',
        'bank_transfer': 'Bank Transfer',
        'check': 'Cheque',
        'page': 'Page',
        'of': 'of',
    },
    'en-US': {
        'invoice': 'Invoice',
        'purchase_order': 'Purchase Order',
        'receipt': 'Receipt',
        'order': 'Order',
        'delivery_note': 'Delivery Note',
        'payslip': 'Payslip',
        'employment_contract': 'Employment Contract',
        'expense_report': 'Expense Report',
        'invoice_number': 'Invoice No.',
        'po_number': 'PO No.',
        'receipt_number': 'Receipt No.',
        'order_number': 'Order No.',
        'delivery_note_number': 'Delivery Note No.',
        'payslip_number': 'Payslip No.',
        'contract_number': 'Contract No.',
        'report_number': 'Report No.',
        'date': 'Date',
        'due_date': 'Due Date',
        'order_date': 'Order Date',
        'delivery_date': 'Delivery Date',
        'bill_to': 'Bill To',
        'ship_to': 'Ship To',
        'vendor': 'Vendor',
        'customer': 'Customer',
        'receiver': 'Receiver',
        'description': 'Description',
        'quantity': 'Quantity',
        'unit': 'Unit',
        'unit_price': 'Unit Price',
        'amount': 'Amount',
        'total': 'Total',
        'subtotal': 'Subtotal',
        'tax': 'Sales Tax',
        'tax_rate': 0.08,
        'grand_total': 'Grand Total',
        'status': 'Status',
        'payment_method': 'Payment Method',
        'items': 'Items',
        'order_items': 'Order Items',
        'items_shipped': 'Items Shipped',
        'weight': 'Weight',
        'package_number': 'Package No.',
        'phone': 'Phone',
        'email': 'Email',
        'receipt': 'Receipt',
        'category': 'Category',
        'sample_watermark': 'SAMPLE - FOR DEMONSTRATION PURPOSES ONLY',
        'thank_you': 'Thank you for your business',
        'company_info': 'Company Information',
        'customer_info': 'Customer Information',
        'pending': 'Pending',
        'approved': 'Approved',
        'received': 'Received',
        'confirmed': 'Confirmed',
        'processing': 'Processing',
        'shipped': 'Shipped',
        'submitted': 'Submitted',
        'rejected': 'Rejected',
        'reimbursed': 'Reimbursed',
        'permanent': 'Permanent',
        'fixed_term': 'Fixed Term',
        'internship': 'Internship',
        'cash': 'Cash',
        'credit_card': 'Credit Card',
        'bank_transfer': 'Wire Transfer',
        'check': 'Check',
        'page': 'Page',
        'of': 'of',
    },
    'es': {
        'invoice': 'Factura',
        'purchase_order': 'Orden de Compra',
        'receipt': 'Recibo',
        'order': 'Pedido',
        'delivery_note': 'Albarán',
        'payslip': 'Nómina',
        'employment_contract': 'Contrato de Trabajo',
        'expense_report': 'Informe de Gastos',
        'invoice_number': 'Nº Factura',
        'po_number': 'Nº Pedido',
        'receipt_number': 'Nº Recibo',
        'order_number': 'Nº Pedido',
        'delivery_note_number': 'Nº Albarán',
        'payslip_number': 'Nº Nómina',
        'contract_number': 'Nº Contrato',
        'report_number': 'Nº Informe',
        'date': 'Fecha',
        'due_date': 'Fecha de Vencimiento',
        'order_date': 'Fecha del Pedido',
        'delivery_date': 'Fecha de Entrega',
        'bill_to': 'Facturar a',
        'ship_to': 'Enviar a',
        'vendor': 'Proveedor',
        'customer': 'Cliente',
        'receiver': 'Destinatario',
        'description': 'Descripción',
        'quantity': 'Cantidad',
        'unit': 'Unidad',
        'unit_price': 'Precio Unitario',
        'amount': 'Importe',
        'total': 'Total',
        'subtotal': 'Subtotal',
        'tax': 'IVA',
        'tax_rate': 0.21,
        'grand_total': 'Total General',
        'status': 'Estado',
        'payment_method': 'Método de Pago',
        'items': 'Artículos',
        'order_items': 'Artículos del Pedido',
        'items_shipped': 'Artículos Enviados',
        'weight': 'Peso',
        'package_number': 'Nº Paquete',
        'phone': 'Teléfono',
        'email': 'Email',
        'receipt': 'Recibo',
        'category': 'Categoría',
        'sample_watermark': 'MUESTRA - SOLO PARA FINES DE DEMOSTRACIÓN',
        'thank_you': 'Gracias por su confianza',
        'company_info': 'Información de la Empresa',
        'customer_info': 'Información del Cliente',
        'pending': 'Pendiente',
        'approved': 'Aprobado',
        'received': 'Recibido',
        'confirmed': 'Confirmado',
        'processing': 'En Proceso',
        'shipped': 'Enviado',
        'submitted': 'Enviado',
        'rejected': 'Rechazado',
        'reimbursed': 'Reembolsado',
        'permanent': 'Indefinido',
        'fixed_term': 'Temporal',
        'internship': 'Prácticas',
        'cash': 'Efectivo',
        'credit_card': 'Tarjeta de Crédito',
        'bank_transfer': 'Transferencia Bancaria',
        'check': 'Cheque',
        'page': 'Página',
        'of': 'de',
    },
    'fr': {
        'invoice': 'Facture',
        'purchase_order': 'Bon de Commande',
        'receipt': 'Reçu',
        'order': 'Commande',
        'delivery_note': 'Bon de Livraison',
        'payslip': 'Fiche de Paie',
        'employment_contract': 'Contrat de Travail',
        'expense_report': 'Note de Frais',
        'invoice_number': 'N° Facture',
        'po_number': 'N° Commande',
        'receipt_number': 'N° Reçu',
        'order_number': 'N° Commande',
        'delivery_note_number': 'N° Bon de Livraison',
        'payslip_number': 'N° Fiche de Paie',
        'contract_number': 'N° Contrat',
        'report_number': 'N° Note de Frais',
        'date': 'Date',
        'due_date': 'Date d\'échéance',
        'order_date': 'Date de commande',
        'delivery_date': 'Date de livraison',
        'bill_to': 'Facturé à',
        'ship_to': 'Livré à',
        'vendor': 'Fournisseur',
        'customer': 'Client',
        'receiver': 'Destinataire',
        'description': 'Description',
        'quantity': 'Quantité',
        'unit': 'Unité',
        'unit_price': 'Prix unitaire',
        'amount': 'Montant',
        'total': 'Total',
        'subtotal': 'Sous-total',
        'tax': 'TVA',
        'tax_rate': 0.20,
        'grand_total': 'Total TTC',
        'status': 'Statut',
        'payment_method': 'Mode de paiement',
        'items': 'Articles',
        'order_items': 'Articles de la Commande',
        'items_shipped': 'Articles Livrés',
        'weight': 'Poids',
        'package_number': 'N° Colis',
        'phone': 'Téléphone',
        'email': 'Email',
        'receipt': 'Reçu',
        'category': 'Catégorie',
        'sample_watermark': 'ÉCHANTILLON - À DES FINS DE DÉMONSTRATION UNIQUEMENT',
        'thank_you': 'Merci pour votre confiance',
        'company_info': 'Informations entreprise',
        'customer_info': 'Informations client',
        'pending': 'En attente',
        'approved': 'Approuvé',
        'received': 'Reçu',
        'confirmed': 'Confirmé',
        'processing': 'En cours de traitement',
        'shipped': 'Expédié',
        'submitted': 'Soumis',
        'rejected': 'Rejeté',
        'reimbursed': 'Remboursé',
        'permanent': 'CDI',
        'fixed_term': 'CDD',
        'internship': 'Stage',
        'cash': 'Espèces',
        'credit_card': 'Carte bancaire',
        'bank_transfer': 'Virement bancaire',
        'check': 'Chèque',
        'page': 'Page',
        'of': 'sur',
    },
    'de': {
        'invoice': 'Rechnung',
        'purchase_order': 'Bestellung',
        'receipt': 'Quittung',
        'order': 'Bestellung',
        'delivery_note': 'Lieferschein',
        'payslip': 'Gehaltsabrechnung',
        'employment_contract': 'Arbeitsvertrag',
        'expense_report': 'Spesenabrechnung',
        'invoice_number': 'Rechnungsnr.',
        'po_number': 'Bestellnr.',
        'receipt_number': 'Quittungsnr.',
        'order_number': 'Bestellnr.',
        'delivery_note_number': 'Lieferscheinnr.',
        'payslip_number': 'Abrechnungsnr.',
        'contract_number': 'Vertragsnr.',
        'report_number': 'Abrechnungsnr.',
        'date': 'Datum',
        'due_date': 'Fälligkeitsdatum',
        'order_date': 'Bestelldatum',
        'delivery_date': 'Lieferdatum',
        'bill_to': 'Rechnung an',
        'ship_to': 'Lieferung an',
        'vendor': 'Lieferant',
        'customer': 'Kunde',
        'receiver': 'Empfänger',
        'description': 'Beschreibung',
        'quantity': 'Menge',
        'unit': 'Einheit',
        'unit_price': 'Einzelpreis',
        'amount': 'Betrag',
        'total': 'Gesamt',
        'subtotal': 'Zwischensumme',
        'tax': 'MwSt',
        'tax_rate': 0.19,
        'grand_total': 'Gesamtbetrag',
        'status': 'Status',
        'payment_method': 'Zahlungsart',
        'items': 'Artikel',
        'order_items': 'Bestellartikel',
        'items_shipped': 'Versendete Artikel',
        'weight': 'Gewicht',
        'package_number': 'Paketnr.',
        'phone': 'Telefon',
        'email': 'E-Mail',
        'receipt': 'Quittung',
        'category': 'Kategorie',
        'sample_watermark': 'MUSTER - NUR FÜR DEMONSTRATIONSZWECKE',
        'thank_you': 'Vielen Dank für Ihr Vertrauen',
        'company_info': 'Firmeninformationen',
        'customer_info': 'Kundeninformationen',
        'pending': 'Ausstehend',
        'approved': 'Genehmigt',
        'received': 'Empfangen',
        'confirmed': 'Bestätigt',
        'processing': 'In Bearbeitung',
        'shipped': 'Versandt',
        'submitted': 'Eingereicht',
        'rejected': 'Abgelehnt',
        'reimbursed': 'Erstattet',
        'permanent': 'Unbefristet',
        'fixed_term': 'Befristet',
        'internship': 'Praktikum',
        'cash': 'Bargeld',
        'credit_card': 'Kreditkarte',
        'bank_transfer': 'Banküberweisung',
        'check': 'Scheck',
        'page': 'Seite',
        'of': 'von',
    },
    'it': {
        'invoice': 'Fattura',
        'purchase_order': 'Ordine di Acquisto',
        'receipt': 'Ricevuta',
        'order': 'Ordine',
        'delivery_note': 'Bolla di Consegna',
        'payslip': 'Busta Paga',
        'employment_contract': 'Contratto di Lavoro',
        'expense_report': 'Nota Spese',
        'invoice_number': 'N° Fattura',
        'po_number': 'N° Ordine',
        'receipt_number': 'N° Ricevuta',
        'order_number': 'N° Ordine',
        'delivery_note_number': 'N° Bolla',
        'payslip_number': 'N° Busta Paga',
        'contract_number': 'N° Contratto',
        'report_number': 'N° Rapporto',
        'date': 'Data',
        'due_date': 'Data di Scadenza',
        'order_date': 'Data Ordine',
        'delivery_date': 'Data di Consegna',
        'bill_to': 'Fatturare a',
        'ship_to': 'Spedire a',
        'vendor': 'Fornitore',
        'customer': 'Cliente',
        'receiver': 'Destinatario',
        'description': 'Descrizione',
        'quantity': 'Quantità',
        'unit': 'Unità',
        'unit_price': 'Prezzo Unitario',
        'amount': 'Importo',
        'total': 'Totale',
        'subtotal': 'Subtotale',
        'tax': 'IVA',
        'tax_rate': 0.22,
        'grand_total': 'Totale Generale',
        'status': 'Stato',
        'payment_method': 'Metodo di Pagamento',
        'items': 'Articoli',
        'order_items': 'Articoli Ordinati',
        'items_shipped': 'Articoli Spediti',
        'weight': 'Peso',
        'package_number': 'N° Pacco',
        'phone': 'Telefono',
        'email': 'Email',
        'receipt': 'Ricevuta',
        'category': 'Categoria',
        'sample_watermark': 'CAMPIONE - SOLO PER SCOPI DIMOSTRATIVI',
        'thank_you': 'Grazie per la vostra fiducia',
        'company_info': 'Informazioni Azienda',
        'customer_info': 'Informazioni Cliente',
        'pending': 'In Attesa',
        'approved': 'Approvato',
        'received': 'Ricevuto',
        'confirmed': 'Confermato',
        'processing': 'In Elaborazione',
        'shipped': 'Spedito',
        'submitted': 'Inviato',
        'rejected': 'Rifiutato',
        'reimbursed': 'Rimborsato',
        'permanent': 'Indeterminato',
        'fixed_term': 'Determinato',
        'internship': 'Stage',
        'cash': 'Contanti',
        'credit_card': 'Carta di Credito',
        'bank_transfer': 'Bonifico Bancario',
        'check': 'Assegno',
        'page': 'Pagina',
        'of': 'di',
    }
}

def get_translation(key, language='en-GB'):
    """Get translation for a key in specified language"""
    return TRANSLATIONS.get(language, TRANSLATIONS['en-GB']).get(key, key)

def format_date(date, language='en-GB'):
    """Format date according to locale"""
    formats = {
        'en-GB': '%d/%m/%Y',  # 31/12/2024
        'en-US': '%m/%d/%Y',  # 12/31/2024
        'fr': '%d/%m/%Y',     # 31/12/2024
        'de': '%d.%m.%Y',     # 31.12.2024
        'es': '%d/%m/%Y',     # 31/12/2024
    }
    return date.strftime(formats.get(language, '%d/%m/%Y'))

def format_currency(amount, language='en-GB'):
    """Format currency according to locale"""
    if language == 'en-GB':
        return f"£{amount:,.2f}"
    elif language == 'en-US':
        return f"${amount:,.2f}"
    elif language == 'fr':
        return f"{amount:,.2f} €".replace(',', ' ')
    elif language == 'de':
        return f"{amount:,.2f} €".replace(',', '.')
    elif language == 'es':
        return f"{amount:,.2f} €".replace(',', '.')
    return f"£{amount:,.2f}"

def get_tax_rate(language='en-GB'):
    """Get tax rate for country"""
    rates = {
        'en-GB': 0.20,   # 20% VAT in UK
        'en-US': 0.08,   # 8% Sales Tax (average)
        'fr': 0.20,      # 20% TVA in France
        'de': 0.19,      # 19% MwSt in Germany
        'es': 0.21,      # 21% IVA in Spain
        'it': 0.22,      # 22% IVA in Italy
    }
    return rates.get(language, 0.20)

def get_currency_code(language='en-GB'):
    """Get currency code for language"""
    currencies = {
        'en-GB': 'GBP',
        'en-US': 'USD',
        'fr': 'EUR',
        'de': 'EUR',
        'es': 'EUR',
        'it': 'EUR',
    }
    return currencies.get(language, 'GBP')
